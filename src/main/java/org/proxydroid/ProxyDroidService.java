...
public class ProxyDroidService extends Service {
    ...
    private int pacRenewDuration = 0;
    ...
    private void enableProxy() {
        ...
        try {
            ...
            if (isPAC) // byPass PAC File IP
                cmd.append(CMD_IPTABLES_RETURN.replace("0.0.0.0", "172.116.0.1"));
...
//    notification.defaults |= Notification.DEFAULT_LIGHTS;
    ...
    private boolean getAddress() {
        ...
        if (isPAC) {
            try {
                ...
                PacScriptSource src = new UrlPacScriptSource(pacHost);
    ...
    public void onStart(Intent intent, int startId) {
        ...
        if (isPAC && pacRenewDuration > 0 && pacRenewDuration < 600) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(), " PAC File Renew Duration: " + pacRenewDuration, Toast.LENGTH_SHORT).show();
                }
            });
            ...
            }, pacRenewDuration * 60 * 1000, pacRenewDuration * 60 * 1000);
        }
